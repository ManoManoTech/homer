...
describe('release > createRelease when switching project in the modal', () => {
  it('creates the release with a tag of the selected project when Slack keeps the previous-tag select state', async () => {
    ...
    const initialPreviousTag = findSelect(
      initialView,
    ...
    const rebuiltPreviousTag = findSelect(
      rebuiltView,
      'release-previous-tag-block',
    );
    ...
    // Same block_id and action_id as the initial view: Slack keeps the value
    // already held in state (ban-offer-history's tag) rather than applying
    // the new initial_option.
